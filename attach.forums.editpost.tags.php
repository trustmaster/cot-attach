...
File=attach.forums.editpost.tags
Hooks=forums.editpost.tags
Tags=forums.editpost.tpl:{FORUMS_EDITPOST_ATTACH}
Order=10
[END_SED_EXTPLUGIN]
==================== */
...
if($cfg['plugin']['attach']['forums'] && sed_auth('plug', 'attach', 'W'))
{
	...
	$t1 = new XTemplate(sed_skinfile('attach.forums.editpost', true));
	...
	$att_sql = sed_sql_query("SELECT att_id, att_path, att_title
		FROM $db_attach WHERE att_type = 'frm' AND att_item = $p
	...
	$t->assign('FORUMS_EDITPOST_ATTACH', $t1->text('MAIN'));
}
?>